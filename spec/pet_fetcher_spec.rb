  ...
  describe '.get_petfinder_pet' do
    it 'returns a hash of pet data when the API request is successful' do
      VCR.use_cassette('petfinder', record: :once) do
        pet = CutePets::PetFetcher.get_petfinder_pet
        pet.description.must_equal 'altered male ferret'
        pet.pic.must_equal 'http://photos.petfinder.com/photos/pets/30078059/1/?bust=1409196072&width=500&-x.jpg'
        pet.link.must_equal 'https://www.petfinder.com/petdetail/30078059'
        pet.name.must_equal 'Joey'
      end
    end
    ...
    it 'returns a hash of pet data when the request is successful' do
      CutePets::PetFetcher.stub(:get_petharbor_pet_type, 'dog') do
        VCR.use_cassette('petharbor', record: :once) do
          pet = CutePets::PetFetcher.get_petharbor_pet
          pet.description.must_equal 'neutered male white bichon frise'
          pet.pic.must_equal 'http://www.PetHarbor.com/get_image.asp?RES=Thumb&ID=A223117&LOCATION=DNVR'
          pet.link.must_equal 'http://www.PetHarbor.com/detail.asp?ID=A223117&LOCATION=DNVR&searchtype=rnd&shelterlist=\'DNVR\'&where=dummy&kiosk=1'
          pet.name.must_equal 'Morty'
        end
      end
    end
  end
